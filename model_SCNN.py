...
class SCNN(nn.Module):
    ...
    def forward(self, img, seg_gt=None, exist_gt=None):
        ...
        x = self.layer2(x) #torch.Size([32, 5, 36, 64])

        seg_pred = F.interpolate(x, scale_factor=8, mode='bilinear', align_corners=True) #([32, 5, 288, 512])
        x = self.layer3(x) #([32, 5, 18, 32])
        x = x.view(-1, self.fc_input_feature) # self.fc_input_feature = 2880
        # x_shape: [16, 2880]
        exist_pred = self.fc(x)
        
        if seg_gt is not None and exist_gt is not None: #seg_gt = ([16, 288, 512])
            # seg_pred:[16, 5, 288, 512]
            # seg_gt:[16, 288, 512]
            # exist_pred:[16, 4]
            # exist_gt:[16, 4]
